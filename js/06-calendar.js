...
function renderMatrix(C){
  ...
  /* ★ v9.6 — BỎ vạch chuyển cơ cấu và nhãn nhóm "A→D".
     Chúng là tàn dư của màn Tái cơ cấu (js/24-reorg.js, đã xoá): một người
     có thể mang nhãn "A→D" trong khi cột Tổ lại ghi B, và ngày áp dụng có
     viền đậm không rõ nghĩa gì — nhìn bảng không ai biết ai đang ở nhóm nào.
     Nhóm nay khai ở đúng một chỗ (màn Ca chuẩn, js/28-stdshift.js) nên cột
     Tổ chỉ còn ghi đúng cái tên nhóm hiện tại. */
  const cutCls=()=>'';
  const evTit=iso=>{
    if(!evOn||!eventsOfDay(iso).length)return '';
    return ` title="${esc('📌 '+evTitleOfDay(iso))}"`;
  };
  ...
  /* ============================================================
     ★ v9.7 — DÒNG TRỐNG PHẢI TỰ NÓI VÌ SAO
     ------------------------------------------------------------
     Một người có mặt trong bảng mà cả kỳ không một ô nào là chuyện BẤT
     THƯỜNG — nhưng trước bản này bảng lịch vẽ nó y như mọi dòng khác, chỉ
     là rỗng. Người dùng bấm "Tạo ca chuẩn" thấy toast báo thành công, quay
     ra bảng vẫn thấy dòng trắng, và không có gì để lần theo.

     Màn Ca chuẩn có chẩn đoán (stdWhyBlank) từ v9.6.1, nhưng nó ở TAB
     KHÁC. Lỗi hiện ra ở đây thì lời giải thích cũng phải ở đây.
     ============================================================ */
  const blankWhy={};
  emps.forEach(e=>{
    if(days.some(iso=>getCode(e.id,iso).code))return;      // có ô → bình thường
    const w=(typeof stdWhyBlank==='function')?stdWhyBlank(e,days):'';
    blankWhy[e.id]=w||'empty';
  });
  const blankBad=Object.keys(blankWhy).filter(id=>{
    const w=blankWhy[id];
    return !(typeof STD_WHY==='object'&&STD_WHY[w]&&STD_WHY[w].bad===false);
  });
  ...
  teamsInOrder.forEach(tm=>{
    ...
    mem.forEach((e,i)=>{
      ...
      /* Dòng cả kỳ không có ô nào → gắn lý do ngay cạnh tên, bấm là sang
         màn Ca chuẩn để sửa. Lý do "đúng như khai" (đặt không xếp lịch,
         ngoài biên chế) vẫn hiện nhưng màu xám, không kêu như lỗi. */
      const bw=blankWhy[e.id]||'';
      const bwInfo=(bw&&typeof STD_WHY==='object'&&STD_WHY[bw])?STD_WHY[bw]:null;
      const bwBad=!!(bw&&(!bwInfo||bwInfo.bad!==false));
      const bwChip=bw?`<span class="blank-why${bwBad?' bad':''}" title="${
        esc(t('Cả kỳ không có ô lịch nào — bấm để sang màn Ca chuẩn sửa'))}"
        onclick="go('setup')">${bwInfo?bwInfo.ic:'⚠'} ${
        esc(t(bwInfo?bwInfo.l:'Không có ô lịch nào'))}</span>`:'';
      h+=`<tr${bw?(' class="blank-row'+(bwBad?' bad':'')+'"'):''}>`+
        `<td class="c0">${i+1}</td>`+
        `<td class="c1" style="background:${col}">${esc(teamShort(tm))}</td>`+
        `<td class="c2">${esc(e.id)}</td>`+
        `<td class="c3">${esc(e.name||'(chưa đặt tên)')}${bwChip}</td>`+
  ...
  h+='</tr></tfoot></table>';
  /* Băng nhắc đặt TRƯỚC bảng: một dòng trống nằm giữa ba mươi dòng đầy thì
     mắt không bắt được, nhất là khi bảng phải cuộn ngang. */
  const warn=blankBad.length?`<div class="mtx-blank">⛔ ${
    t('Không có ô lịch nào cả kỳ')}: ${blankBad.map(id=>{
      const e=emps.find(x=>x.id===id)||{};
      const w=blankWhy[id],inf=(typeof STD_WHY==='object'&&STD_WHY[w])?STD_WHY[w]:null;
      return esc(e.name||id)+' ('+esc(t(inf?inf.l:'chưa rõ'))+')';
    }).join(' · ')} <button class="btn warn sm" onclick="go('setup')">${
    t('Mở màn Ca chuẩn để sửa')}</button></div>`:'';
  $(C.box).innerHTML=warn+h;
}
...
function refreshHoldBtn(){
  /* ★ v9.5 — nút Áp pattern đi cùng điều kiện: cả hai chỉ có nghĩa ở LỊCH
     THỰC TẾ. Ca chuẩn là 4 nhóm cố định, không nhận pattern và không sinh
     thông báo. Xem js/27-pattern.js. */
  const pb=$('patBtn');
  const show=canEditSched()&&calTab==='sched'&&calMode==='real';
  if(pb)pb.style.display=(show&&typeof openPatBox==='function')?'':'none';
