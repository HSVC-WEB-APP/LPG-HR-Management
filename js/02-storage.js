...
/* ★ v9.6 — nhánh 'reorgs' đã bỏ cùng màn Tái cơ cấu. Dữ liệu cũ trên
   Firebase (nếu còn) được stdMigrateV96() dọn lúc boot; không nghe, không
   ghi nữa. Xem js/28-stdshift.js. */
...
function normalizeState(){
  ...
  /* ★ v9.6 — sổ structLog / planSegs của các bản cũ đã bỏ; stdMigrateV96()
     dọn nốt lúc boot. Khai báo nhóm nay nằm ở S.settings.stdTeams, do
     js/28-stdshift.js tự dựng khi cần. */
...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   ★ v9.7 — BIA MỘ KHÔNG ĐƯỢC CHÔN NGƯỜI CÒN SỐNG
   ------------------------------------------------------------
   TRIỆU CHỨNG người dùng gặp: bấm "Tạo ca chuẩn", toast báo đã tạo đủ ô,
   mà dòng của một người vẫn TRỐNG TRƠN cả kỳ — và màn Ca chuẩn không kể
   tên anh ta ra, vì theo mọi phép kiểm thì anh ta xếp được bình thường.

   GỐC RỄ. Ba nhánh dữ liệu dưới đây lấy MÃ NHÂN VIÊN làm khoá:

       base/<mã NV>      ca chuẩn
       over/<mã NV>      ca thực tế
       accounts/<mã NV>  tài khoản đăng nhập

   Các nhánh còn lại (requests, notifs, trainings…) khoá bằng id riêng của
   bản ghi — id ấy sinh một lần rồi chết hẳn, nên bia mộ vĩnh viễn là đúng.
   Mã nhân viên thì KHÔNG: xoá một người rồi khai lại chính mã NV đó là
   việc bình thường của phòng nhân sự (khai sai, nhập trùng, người cũ quay
   lại). Mà `changeId()` cũng đi qua đúng đường ấy — đổi mã A→B rồi đổi
   ngược B→A là đủ để mã A có bia mộ.

   Khi đó vòng đời một lần bấm Tạo là:
       stdBuild()  ghi S.base[mã] = 31 ô
       save() → fbPush() → applyTombstones()  thấy bia mộ base/<mã>
                                              → XOÁ SẠCH 31 ô vừa ghi
       renderBoth()                           → dòng trống
   Không một lời báo lỗi nào, vì đứng ở góc nhìn của sổ bia mộ thì nó đang
   làm đúng việc được giao.

   CÁCH CHỮA. Một bất biến, phát biểu được bằng một câu:

       Mã NV có mặt trong S.employees là NGƯỜI CÒN SỐNG.
       Bia mộ của người còn sống là bia mộ LẠC — gỡ, đừng thi hành.

   Nên `applyTombstones()` không còn xoá mù quáng: với ba nhánh khoá theo
   mã NV, nó đối chiếu danh sách nhân viên trước. Bia mộ lạc bị GỠ HẲN
   khỏi sổ (không chỉ bỏ qua) để lần đồng bộ sau không phải xử lại, và để
   máy khác cũng nhận được sổ đã sạch.
   ============================================================ */
const EMP_KEYED_BRANCHES=['base','over','accounts'];
/* Gỡ bia mộ của một khoá. Trả true nếu có gỡ được cái nào. */
function tombLift(branch,id){
  if(!S.del||!S.del[branch])return false;
  if(S.del[branch][id]===undefined)return false;
  delete S.del[branch][id];
  if(!Object.keys(S.del[branch]).length)delete S.del[branch];
  return true;
}
/* Gỡ mọi bia mộ mang mã NV này — gọi khi một người được KHAI LẠI. */
function tombLiftEmp(id){
  let n=0;
  EMP_KEYED_BRANCHES.forEach(b=>{if(tombLift(b,id))n++;});
  return n;
}
/* Quét cả sổ: mã NV nào đang có người thì gỡ bia mộ của mã ấy.
   Rẻ (vài chục phần tử) nên gọi được ở mọi lối vào ghi dữ liệu. */
function tombLiftLiving(){
  if(!S.del||!Array.isArray(S.employees))return 0;
  let n=0;
  S.employees.forEach(e=>{if(e&&e.id)n+=tombLiftEmp(e.id);});
  return n;
}
/* Người còn sống nào đang bị bia mộ đè? Dùng cho thẻ tự kiểm ở màn quản trị. */
function tombLivingHits(){
  const out=[];
  if(!S.del||!Array.isArray(S.employees))return out;
  S.employees.forEach(e=>{
    if(!e||!e.id)return;
    const br=EMP_KEYED_BRANCHES.filter(b=>tombHas(b,e.id));
    if(br.length)out.push({id:e.id,name:e.name||e.id,branches:br});
  });
  return out;
}
...
function applyTombstones(){
  if(!S.del)return 0;
  tombMigrate();
  /* ★ v9.7 — GỠ BIA MỘ LẠC TRƯỚC KHI THI HÀNH BẤT CỨ CÁI NÀO.
     Mã NV đang có người là người còn sống; bia mộ của họ là rác sót lại
     từ một lần xoá / đổi mã trước đó. Gỡ trước, rồi mới xoá theo sổ —
     làm ngược thứ tự là xoá mất dữ liệu vừa ghi. */
  tombLiftLiving();
  let n=0;
  FB_MAP_BRANCHES.forEach(k=>{
...
function fbDiff(prev,cur){
  ...
  FB_MAP_BRANCHES.forEach(k=>{
    ...
    Object.keys(a).forEach(id=>{if(b[id]===undefined){
      patch[k+'/'+id]=null;n++;
      /* ★ v9.7 — KHÔNG dựng bia mộ cho MÃ NV ĐANG CÓ NGƯỜI.
         Ba nhánh base / over / accounts khoá theo mã nhân viên, và app xoá
         nguyên map của một người ở nhiều chỗ HOÀN TOÀN BÌNH THƯỜNG mà
         người ấy vẫn còn trong biên chế — rõ nhất là stdToggleSched()
         (js/28-stdshift.js): đặt "không xếp lịch" chạy `delete S.base[id]`.
         Bật lại thì lịch xếp được như thường, nhưng bia mộ dựng lúc tắt vẫn
         nằm đó và xoá sạch mọi ô vừa ghi. Đây chính là đường đi tới một
         dòng lịch trống mà không lời báo nào.
         applyTombstones() đã gỡ bia mộ lạc; chặn thêm ở đây để nó không
         bao giờ được dựng lên, khỏi phải dọn và khỏi lan sang máy khác. */
      if(EMP_KEYED_BRANCHES.indexOf(k)>=0&&Array.isArray(S.employees)
         &&S.employees.some(e=>e&&e.id===id))return;
...
/* ============================================================
   ★ v9.6 — DỌN DỮ LIỆU CŨ SAU KHI DỮ LIỆU MÁY CHỦ VỀ
   ------------------------------------------------------------
   Migration ở js/12-main.js chạy lúc BOOT, tức là TRƯỚC khi fbBootLoad()
   thay nguyên `S.employees` bằng bản của máy chủ. Bản máy chủ còn ở dạng cũ
   (nhóm ảo "DCS"/"Field", dải chặng e.segs…) nên nó ghi đè lại kết quả dọn
   — đúng cái đã xảy ra: xoá nhóm DCS xong tải lại là nó hiện ra y như cũ.

   Nên phải dọn LẦN NỮA ở đây, và quan trọng là dọn SAU khi mốc đồng bộ
   (_fbLast) đã được đặt theo đúng bản máy chủ — có thế save() mới thấy khác
   biệt mà đẩy bản đã dọn lên cho mọi máy.
   ============================================================ */
function stdCleanAfterRemote(){
  if(typeof stdMigrateV96!=='function')return 0;
  let n=stdMigrateV96();
  if(typeof stdSyncAll==='function')n+=stdSyncAll();
  if(typeof stdLeaverSweep==='function')n+=stdLeaverSweep();
  if(typeof stdPurgeAfterLeft==='function')n+=stdPurgeAfterLeft();
  if(n)save();
  return n;
}
...
function fbBootLoad(){
  ...
  fbRef.once('value').then(snap=>{
    ...
    /* ★ v9.6 — dữ liệu máy chủ có thể còn ở dạng cũ. Dọn ở ĐÂY, sau khi
       _fbLast đã đặt theo bản máy chủ, để save() đẩy bản đã dọn lên. */
    stdCleanAfterRemote();
    ...
    /* ★ v9.9 — bản tin HR (đơn đã duyệt chưa tích "đã nhập HR"). Mốc ngày
       riêng nên phải gọi riêng; xem js/21-notify.js. */
    try{ if(typeof hrPendFlush==='function')hrPendFlush(); }catch(e){}
    if(typeof renderAll==='function')renderAll();
    setSync(true,'Đã đồng bộ');
...
function fbListen(){
  ...
  FB_VAL_BRANCHES.forEach(k=>{
    fbRef.child(k).on('value',snap=>{
      ...
      /* Máy khác đẩy lên bản nhân sự còn ở dạng cũ → dọn rồi đẩy lại.
         fbMark() ở trên đã ghi nhận bản THÔ làm mốc nên save() thấy khác. */
      if(k==='employees')stdCleanAfterRemote();
      fbTouch();
      setSync(true,'Đã đồng bộ');
    },fbErr);
  });
