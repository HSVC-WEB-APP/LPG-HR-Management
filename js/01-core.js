...
/* ★ v8.9 — NGÀY ĐÓ NGƯỜI NÀY CÒN TRONG BIÊN CHẾ KHÔNG?
   `joinAt` = ngày vào làm, `leftAt` = ngày làm việc CUỐI CÙNG. Cả hai khai
   ở dải "vòng đời" trên từng dòng người của màn Ca chuẩn (js/28-stdshift.js)
   — PHẢI có chỗ nhìn thấy chúng, vì chúng cắt lịch: một trường vô hình mà
   điều khiển kết quả là cách chắc nhất để sinh ra một dòng lịch trống mà
   không ai hiểu vì sao. Người đã nghỉ việc KHÔNG bị xoá khỏi
...
/* Có ít nhất MỘT ngày trong khoảng mà người này còn trong biên chế?
   (dùng để lọc DÒNG của bảng lịch: người nghỉ việc giữa kỳ vẫn phải hiện ở
   kỳ đó, sang kỳ sau mới biến mất.)

   ★ v9.6.1 — phải tính bằng GIAO của hai khoảng, không phải kiểm hai đầu
   rời nhau. Bản cũ kiểm riêng "joinAt sau ngày cuối?" và "leftAt trước ngày
   đầu?", nên một người có `joinAt` 08/09 và `leftAt` 06/09 (dữ liệu cũ sót
   lại, vào làm SAU ngày nghỉ) lọt qua cả hai phép kiểm → hàm này nói CÒN
   trong biên chế, trong khi inServiceOn() nói KHÔNG cho từng ngày một.
   Hai hàm cùng một khái niệm mà nói ngược nhau: bảng lịch vẽ ra một dòng
   trống trơn, không một lời giải thích. */
function inServiceRange(e,fromIso,toIso){
  if(!e)return false;
  const f=(e.joinAt&&(!fromIso||e.joinAt>fromIso))?e.joinAt:fromIso;
  const t=(e.leftAt&&(!toIso  ||e.leftAt<toIso  ))?e.leftAt:toIso;
  if(!f||!t)return true;                  /* không giới hạn hai đầu thì luôn có */
  return f<=t;
}
...
/* ============================================================
   ...
   Cơ cấu 2 tuyến DCS/Field (đã bỏ ở v9.6) từng phá vỡ giả định đó: nhóm
   "Field" gồm TOÀN Field Engineer, teamFieldEngId() lấy người đầu danh sách,
   thế là một kỹ sư đi duyệt đơn của kỹ sư khác — họ ngang cấp nhau, chuyện
   đó sai về mặt tổ chức. Luật dưới đây sinh ra từ đó và vẫn giữ nguyên.
   ...
   Nhóm nào không còn Field Engineer thì cấp 'fe' tự biến mất — chuỗi rút
   còn Trung → QL người Hàn.
