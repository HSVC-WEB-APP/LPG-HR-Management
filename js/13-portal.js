...
function dsPerDayHtml(kind){
  ...
  const m=(kind==='cover')?dsCoverBy:dsWithBy;
  const nOvr=Object.keys(m||{}).length;
  ...
  /* ★ 2026-09-08 — gập lại mặc định: đa số đơn dùng một người cho mọi ngày,
     ô này chỉ cần khi có ai đó khai khác nhau theo từng ngày. Đã có tùy
     chỉnh thì tự mở sẵn để không giấu mất lựa chọn đang dùng. */
  return `<details class="ds-fold sm"${nOvr?' open':''}>
    <summary>${lb}${nOvr?`<span class="h4c">${t(nOvr+' tùy chỉnh')}</span>`:''}</summary>
    <div class="ds-fold-b">
...
function renderDaySheet(){
  ...
  box.innerHTML=`
   ...
   ${crew.length?`<div class="ds-block">
     <details class="ds-fold"${isMobile()?'':' open'}>
     <summary>👥 Nhân sự ngày ${fmtVN(iso)} <span class="h4n">${totalCrew} người</span>${
        nChanged?`<span class="h4c">${nChanged} khác lịch chuẩn</span>`:''}</summary>
     <div class="ds-fold-b">
     <div class="crew-grid">
       ${crew.map(([b,list])=>{
...
/* Chọn đưa đơn vào hàng chờ in hay không cần in.
   Không in = đơn vẫn được lưu & duyệt bình thường, chỉ không hiện trong danh sách in.
   ★ 2026-09-08 — hai loại multi/swap BẮT BUỘC in thì hiện luôn (quan trọng,
   đừng giấu); năm loại còn lại mặc định đúng theo quy định công ty nên gập
   lại, ai cần đổi mới bấm mở — đỡ chiếm chỗ màn hình điện thoại. */
function dsPrintToggleHtml(){
  const must=REQ_MUST_PRINT.includes(pvSheetForm);
  const body=`<div class="seg print-seg">
  ...
  if(must)return `<div class="fg"><label class="fl">In đơn</label>${body}</div>`;
  return `<details class="ds-fold sm">
    <summary>${t('In đơn')} <span class="h4n">${dsNoPrint?t('Không cần in'):t('Cần in')}</span></summary>
    <div class="ds-fold-b">${body}</div></details>`;
}
