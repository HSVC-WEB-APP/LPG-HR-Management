...
function meId(){
  try{const s=JSON.parse(localStorage.getItem(SESS)||'null');
    ...
    const e=s&&s.id?empById(s.id):null;
    /* ★ v9.6 — người đã NGHỈ VIỆC và đã qua kỳ (active===false) thì phiên
       đang mở cũng hết hiệu lực ngay, không đợi họ đăng xuất. Chặn ở đây là
       chặn ở gốc: mọi màn của app đều hỏi meId() trước khi cho làm gì. */
    if(e&&e.active===false)return null;
    if(s&&s.id&&e&&a&&(a.hash||a.init))return s.id;
  }catch(e){}
  return null;
}
...
async function doLogin(){
  ...
  /* Đã nghỉ việc và đã qua kỳ → không cho vào nữa. Nói thẳng lý do: để họ
     gõ lại mật khẩu ba lần rồi mới hiểu là một cách đối xử tệ. */
  if(e.active===false){
    gateMsg(t('Tài khoản <b>')+esc(key)+t('</b> đã ngừng hoạt động (đã khai nghỉ việc). Liên hệ quản lý nếu đây là nhầm lẫn.'));
    return;
  }
  if(!canHaveAccount(e)){
