...
function digestStartTimer(){
  ...
  /* ★ v9.9 — một nhịp hẹn giờ lo CẢ HAI bản tin. Hai việc độc lập nhau
     (mốc ngày riêng, nguồn dữ liệu riêng) nhưng cùng một giờ bắn, nên
     không có lý do gì đẻ thêm một setInterval thứ hai. */
  const tick=()=>{ try{ digestFlush(); }catch(e){}
                   try{ hrPendFlush(); }catch(e){} };
  _digestTick=setInterval(tick,DIGEST_TICK_MS);
  try{
    document.addEventListener('visibilitychange',function(){
...
/* ============================================================
   ★ v9.9 — BẢN TIN HR MỖI SÁNG: ĐƠN ĐÃ DUYỆT MÀ CHƯA TÍCH "ĐÃ NHẬP HR"
   ------------------------------------------------------------
   VIỆC THẬT

   Đơn duyệt xong trong app vẫn phải được GÕ LẠI vào hệ thống nhân sự của
   công ty. Dấu tick `r.hrAt` (v6.1) ghi lại việc đó, và màn Duyệt có bộ lọc
   "chưa nhập" — nhưng phải có người nhớ mở app, chọn đúng kỳ, chọn đúng bộ
   lọc thì mới thấy. Cuối kỳ chốt công mới phát hiện sót thì đã muộn.

   KHÁC BẢN TIN 08:00 Ở CHỖ NÀO

   Bản tin gom (`digest*`) là SỔ CHỜ SỰ KIỆN: mỗi mục là một thông báo vừa
   sinh ra, gửi xong là xoá, không bao giờ nhắc lại. Bản tin này thì ngược
   lại — nó là một PHÉP QUÉT LẠI `S.requests` mỗi sáng, nên một đơn chưa
   tích HR sẽ được nhắc LẶP LẠI mỗi ngày cho tới khi có người bấm tick. Đó
   chính là thứ cần cho việc đối chiếu cuối kỳ.

   Vì vậy nó KHÔNG dùng chung sổ chờ, không dùng chung mốc ngày, và cũng
   không đi qua `newNotif()` — không có thông báo nào trong app tương ứng
   với nó. Chỉ chung một thứ: giờ bắn (08:00) và cơ chế giành quyền bằng
   transaction để 23 máy cùng mở app không bắn 23 tin.

   PHẠM VI (chốt với Hoàng Trung 08/09/2026)

     · CHỈ đơn đã DUYỆT CHỐT (`status==='approved'`) mà chưa có `hrAt`
       — đơn còn chờ duyệt chưa tới lượt HR; đơn bị từ chối thì HR không gõ.
     · Chỉ đơn có ngày rơi vào KỲ CÔNG đang chạy (21→20).
     · Xếp theo LOẠI ĐƠN, mỗi loại một khối, trong khối liệt kê từng người.
     · Gửi chung lên Zalo bot, ghi rõ là việc của THƯ KÝ.
   ============================================================ */
const HRPEND_HOUR      = DIGEST_HOUR;        // 08:00, cùng nhịp bản tin sáng
const HRPEND_MAX_LINES = DIGEST_MAX_LINES;

/* Kỳ công đang chạy. Ngày ≥21 thuộc kỳ tháng sau — periodFor() lo việc đó. */
function hrPendPeriod(ym){
  if(typeof periodFor!=='function'||typeof curSchedMonth!=='function')return null;
  try{ return periodFor(ym||curSchedMonth()); }catch(e){ return null; }
}
/* Đơn có chạm vào kỳ này không. Đơn "làm liên tục nhiều ngày" là một KHOẢNG
   (from→to) chứ không phải danh sách ngày — xét giao khoảng, không thì đơn
   bắc cầu qua ngày 21 bị rơi mất. */
function hrPendTouches(r,per){
  if(!per)return true;
  if(r.type==='multi'){
    const a=r.from||'', b=r.to||r.from||'';
    return !!(a&&a<=per.to&&b>=per.from);
  }
  const days=dgDays(r);
  if(!days.length){const a=r.from||'';return !!(a&&a>=per.from&&a<=per.to);}
  return days.some(d=>d&&d.iso&&d.iso>=per.from&&d.iso<=per.to);
}
/* Đơn này đang chờ gõ vào hệ thống HR? */
function hrPendIs(r,per){
  if(!r)return false;
  if(r.status!=='approved')return false;                       // chưa chốt / bị từ chối
  if(typeof reqIsProvisional==='function'&&reqIsProvisional(r))return false;
  const done=(typeof reqHrDone==='function')?reqHrDone(r):!!r.hrAt;
  if(done)return false;
  return hrPendTouches(r,per);
}
/* Danh sách đơn chờ nhập HR của kỳ đang chạy, xếp theo loại rồi theo ngày. */
function hrPendList(ym){
  const per=hrPendPeriod(ym);
  const all=(typeof S!=='undefined'&&S.requests)?S.requests:{};
  const out=[];
  Object.keys(all).forEach(id=>{
    const r=all[id];
    if(!r)return;
    if(!hrPendIs(r,per))return;
    out.push(r.id?r:Object.assign({id:id},r));
  });
  const ord=k=>{const i=DG_TYPE_ORDER.indexOf(k);return i<0?99:i;};
  return out.sort((a,b)=>
    (ord(a.type)-ord(b.type)) ||
    String(a.from||'').localeCompare(String(b.from||'')) ||
    String(zName(a.empId)).localeCompare(String(zName(b.empId))));
}
function hrPendCount(ym){return hrPendList(ym).length;}
/* Ai là thư ký — tin ghi đích danh để không thành "việc của cả nhà máy",
   và để sau này có liên kết 1-1 thì gửi thẳng đúng người. */
function hrPendSecs(){
  if(typeof activeEmps!=='function'||typeof permOf!=='function')return [];
  try{ return activeEmps().filter(e=>permOf(e.id)==='sec'); }catch(e){ return []; }
}
/* ------------------------------------------------------------
   THÂN TIN — tiếng Anh, xếp theo LOẠI ĐƠN
   Dùng lại đúng bộ dựng dòng của bản review cấp cuối (dgOneRow /
   dgPadBlock / DG_TYPE_EN) để hai bản tin đọc giống nhau; ở đây mọi đơn
   đều đã duyệt nên không có dấu ⏳ nào.
   ------------------------------------------------------------ */
function hrPendLines(reqs,per){
  const L=[];
  if(per)L.push('Period '+zDate(per.from)+' → '+zDate(per.to));
  const secs=hrPendSecs();
  L.push('For: '+(secs.length?secs.map(e=>zName(e.id)).join(', ')+' (secretary)'
                             :'secretary / HR'));
  L.push('');
  ...
  L.push('SUMMARY');
  const wT=Math.min(20,types.reduce((m,k)=>Math.max(m,(DG_TYPE_EN[k]||String(k)).length),0));
  types.forEach(k=>{
    const nm=(DG_TYPE_EN[k]||String(k).toUpperCase());
    const extra=(k==='ot'&&otH)?('   '+rnd1x(otH)+'h total'):'';
    L.push('  '+(nm.length>=wT?nm:nm+' '.repeat(wT-nm.length))+'  '+byType[k].length+extra);
  });
  L.push('  ── '+reqs.length+' request(s) · '+people+' employee(s) not entered yet');
  ...
  L.push('');
  L.push('All approved. Type them into the company HR system, then tick');
  L.push('"HR entered" in the app (Approve tab) so they drop off this list.');
  if(L.length>HRPEND_MAX_LINES){
    const cut=L.length-HRPEND_MAX_LINES;
    return L.slice(0,HRPEND_MAX_LINES).concat(['','… and '+cut+' more line(s) — open the app to see all']);
  }
  return L;
}
/* Gói một hàng đợi Zalo hoàn chỉnh — trả null khi không có gì để gửi. */
function hrPendItem(ym,qid){
  const per=hrPendPeriod(ym);
  const reqs=hrPendList(ym);
  if(!reqs.length)return null;
  const secs=hrPendSecs();
  /* Khoảng kỳ trong TIÊU ĐỀ viết gọn dd/mm — zDate() có kèm thứ trong tuần,
     đúng cho thân tin nhưng làm tiêu đề dài gấp đôi mà không thêm nghĩa gì. */
  const dm=iso=>String(iso||'').slice(8,10)+'/'+String(iso||'').slice(5,7);
  const span=per?(dm(per.from)+'→'+dm(per.to)):'';
  const item={
    /* `to` để dành cho ngày có liên kết 1-1 (ZALO-BOT.md mục 3.3): lúc ấy
       tin đi thẳng tới thư ký mà không phải sửa lại chỗ này. Hôm nay mọi
       tin vẫn đổ về một chat nhóm, nên bcast=1 để Apps Script bỏ dòng
       "👤 <tên>" — người phải làm đã nằm ngay dòng "For:" trong thân tin. */
    to: secs.length?secs[0].id:'',
    toName: secs.length?zName(secs[0].id):'Secretary',
    title: '📋 HR ENTRY PENDING · '+reqs.length+' request(s)'+(span?(' · '+span):''),
    lines: hrPendLines(reqs,per),
    action: '', group:'hrpend', bcast:1, pri:'batch',
    notifId: qid, n: reqs.length
  };
  item.fp=zaloFp(item);
  return item;
}
/* ------------------------------------------------------------
   BẮN — mỗi ngày MỘT lần, một máy duy nhất.
   Mốc riêng `meta/hrPendDay`, KHÔNG dùng chung `meta/digestDay`: hai bản
   tin độc lập nhau, sổ chờ rỗng không được phép nuốt mất bản tin HR và
   ngược lại.
   ------------------------------------------------------------ */
function hrPendDayKey(dt){return digestDayKey(dt);}
function hrPendDue(now){
  if(!zaloOn())return false;
  const d=now||new Date();
  if(d.getHours()<HRPEND_HOUR)return false;
  return ((S.meta&&S.meta.hrPendDay)||'')!==hrPendDayKey(d);
}
function hrPendStamp(day){
  const d=day||hrPendDayKey();
  S.meta=S.meta||{};S.meta.hrPendDay=d;
  if(typeof fbRef==='undefined'||!fbRef)return;
  fbRef.child('meta').child('hrPendDay').set(d).catch(()=>{});
}
let _hrPendBusy=false;
function hrPendFlush(force,cb){
  const done=ok=>{_hrPendBusy=false;cb&&cb(!!ok);};
  try{
    if(_hrPendBusy){cb&&cb(false);return;}
    if(typeof fbRef==='undefined'||!fbRef){cb&&cb(false);return;}
    if(typeof fbReady==='function'&&!fbReady()){cb&&cb(false);return;}
    if(!force&&!hrPendDue()){cb&&cb(false);return;}
    const day=hrPendDayKey();
    /* Không có đơn nào chờ nhập → KHÔNG gửi tin rỗng (quy tắc R5), nhưng
       vẫn đóng mốc ngày để khỏi quét lại mỗi 5 phút suốt cả ngày. */
    if(!hrPendCount()){ if(!force)hrPendStamp(day); done(true); return; }
    _hrPendBusy=true;
    const prevDay=(S.meta&&S.meta.hrPendDay)||'';
    const send=()=>{
      /* Bấm tay thì thêm đuôi ngẫu nhiên: gửi lại trong cùng một ngày phải
         là một hàng MỚI, không đè lên hàng cũ có thể đang chờ gửi. */
      const qid='hrpend-'+day+(force?('-'+Math.random().toString(36).slice(2,7)):'');
      const it=hrPendItem('',qid);
      if(!it){done(true);return;}
      const row={
        to:it.to, toName:it.toName, title:it.title, lines:it.lines,
        action:'', group:it.group, bcast:1, pri:it.pri,
        notifId:it.notifId, n:it.n, fp:it.fp, hrpend:1,
        state:'pending', createdAt:Date.now()
      };
      let pr;
      try{ pr=fbRef.child('zaloQueue').child(qid).set(row); }
      catch(e){ console.warn('[zalo] gói bản tin HR không hợp lệ',e); done(false); return; }
      pr.then(()=>done(true)).catch(e=>{
        console.warn('[zalo] không ghi được bản tin HR',e);
        /* Ghi trượt thì TRẢ MỐC VỀ để lần mở app sau bắn lại — đúng bài học
           của v6.7: chưa có ack của máy chủ thì chưa được coi là xong. */
        S.meta=S.meta||{};S.meta.hrPendDay=prevDay;
        try{ fbRef.child('meta').child('hrPendDay').set(prevDay||null).catch(()=>{}); }catch(_){}
        done(false);
      });
    };
    if(force){send();return;}
    fbRef.child('meta').child('hrPendDay').transaction(cur=>{
      if(cur===day)return;                 // máy khác vừa bắn xong → bỏ cuộc
      return day;
    },(err,committed)=>{
      if(err||!committed){done(false);return;}
      S.meta=S.meta||{};S.meta.hrPendDay=day;   // xem chú thích ở digestFlush
      send();
    });
  }catch(e){
    console.warn('[zalo] bỏ qua lỗi bản tin HR, app vẫn chạy bình thường',e);
    done(false);
  }
}
/* Bấm tay ở màn Dữ liệu — không đợi 08:00 */
function hrPendSendNow(){
  const n=hrPendCount();
  if(!n){toast(t('Không có đơn nào chờ nhập HR trong kỳ này'));return;}
  if(!confirm(t('Gửi ngay bản tin HR')+' ('+n+' '+t('đơn')+')?'))return;
  hrPendFlush(true,ok=>toast(ok?t('Đã gửi bản tin HR'):t('Chưa gửi được — thử lại sau')));
}
