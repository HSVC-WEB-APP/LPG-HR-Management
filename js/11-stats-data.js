...
function renderAccTbl(){
  ...
  /* ★ v9.6 — bảng này CHỈ còn lo tài khoản. Nhóm · Vị trí · Kiểu ca đã bỏ ô
     nhập: khai ở hai chỗ thì hai chỗ lệch nhau, mà nơi đúng để khai là màn
     Ca chuẩn (js/28-stdshift.js). Ở đây chúng chỉ còn là chữ, để người đặt
     mật khẩu biết mình đang thao tác với ai. Kiểu ca bỏ hẳn — nó là thuộc
     tính của NHÓM, không phải của người. */
  let h=`<thead><tr>
    <th>Mã NV</th><th>Họ tên</th><th>Nhóm</th><th>Vị trí</th>
    <th>Quyền</th><th>Mật khẩu</th><th></th></tr></thead><tbody>`;
  activeEmps().forEach(e=>{
    ...
    h+=`<tr>
      ...
      <td>${e.team?esc(e.team):`<span class="muted">${t('(chưa phân nhóm)')}</span>`}</td>
...
function renderData(){
  ...
  /* ★ v9.9 — bản tin HR: số đơn đã duyệt mà chưa tích "đã nhập HR" (js/21-notify.js) */
  if($('hrPendN')&&typeof hrPendCount==='function'){
    $('hrPendN').textContent=hrPendCount();
    const hl=(S.meta&&S.meta.hrPendDay)||'';
    $('hrPendLast').textContent=hl?('· lần gửi gần nhất: '+hl):'· chưa gửi lần nào';
  }
